...
*SO_REUSEADDR* 允许重用本地地址 
  这个条件通常是这样遇到的：
    a) 启动一个监听服务器
    b) 连接请求到达，派生一个进程来处理客户请求
    c) 服务器进程终止，但是客户请求还没有被处理完。
    d) 服务器进程重启，但是新的服务器进程无法绑定到相同的本地地址和端口，因为该地址和端口已经被前一个服务器进程占用。
    e) 解决方案是设置SO_REUSEADDR选项。
...
*SO_KEEPALIVE* 保持连接 如果在2小时内任一方没有数据交换，就给对端发一个保持存活的探测分节。
*SO_LINGER* 
